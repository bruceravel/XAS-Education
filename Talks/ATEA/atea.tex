...
\date[AEDA2011]{Advanced EXAFS Data Analysis workshop 2011\\
...
\begin{frame}
  ...
  \begin{block}{This is \textbf{NOT} the introductory talk}
    \begin{itemize}
    \item I assume you are a veteran of many XAS experiments and that
...
\begin{frame}
  \frametitle{What is This \textit{Nyquist Criterion}?}
...
\begin{frame}
  ...
  \begin{block}{}
    In Gaussian statistics and with a proper measure of $\epsilon$, a
    \textit{good fit} has $\chi^2\approx1$.
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  {\small
    \begin{align}
      ...
      { \frac{{\color{SlateBlue3}(N_\Gamma S_0^2)}{\color{Cornsilk4}F_\Gamma(k)}}
        {2\,kR_\Gamma^2} }
      e^{i(2kR_\Gamma + {\color{Cornsilk4}\Phi_\Gamma(k)})}
      e^{-2{\color{SlateBlue3}\sigma_\Gamma^2}k^2}
      e^{-2R_\Gamma/{\color{Cornsilk4}\lambda(k)}}
      \Biggl) \\
      \chi_{\mathrm{theory}}(k) =& \sum\limits_{\Gamma}\chi(k,\Gamma) \notag\\
      R_\Gamma =& \> {\color{Cornsilk4}R_{0,\Gamma}} +
...
{\color{Blue4}! At.type   x        y       z       tag}
  ...
  \begin{columns}[B]
    \begin{column}{0.5\linewidth}
      \scriptsize
      \begin{alltt}
  {\color{SteelBlue4}guess}  s0sqr = 0.9
  {\color{Purple4}path} \{
     {\color{Cornsilk4}file}    site1/feff0001.dat
     {\color{Cornsilk4}label}   1st path, site 1
     {\color{Cornsilk4}s02}     s0sqr / 3
   \}
  {\color{Blue4}## all subsequent paths for site 1
  ## have} {\color{Cornsilk4}s02}{\color{Blue4} * 1/3}
      \end{alltt}      
    \end{column}
    \begin{column}{0.5\linewidth}
      \scriptsize
  ...
  {\color{Purple4}path} \{
     {\color{Cornsilk4}file}    site2/feff0001.dat
     {\color{Cornsilk4}label}   1st path, site 2
     {\color{Cornsilk4}s02}     2 * s0sqr / 3
   \}
  {\color{Blue4}## all subsequent paths for site 2
  ## have} {\color{Cornsilk4}s02}{\color{Blue4} * 2/3}
      \end{alltt}      
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\linewidth}
      ...
      \end{itemize}
      \begin{alertblock}{}
        There is no way to find a feff.inp file for that!
      \end{alertblock}
    \end{column}
    \begin{column}{0.5\linewidth}
...
\begin{frame}
  ...
  \item You have data at {\multiple} edges of the same material
  \end{enumerate}
...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    \begin{alltt}
    \scriptsize
     {\color{Purple4}set}      n\_eq     = 6
     {\color{Purple4}guess}    n\_short  = 3
     {\color{Purple4}def}      n\_long   = n\_eq - n\_short
     {\color{Purple4}def}      n\_p      = n\_short
     {\color{Purple4}def}      n\_c      = n\_long / 2

     {\color{Purple4}guess}    ss\_short = 0.003
     {\color{Purple4}def}      ss\_long  = ss\_short
    \end{alltt}
  \end{block}

  We have described the coordination numbers and $\sigma^2$ for the
  equatorial oxygen atoms with a minimal number of
  {\color{Purple4}guess}es.  The constraints on $\sigma^2$ and
  $N_{eq}$ can be lifted easily by switching a {\color{Purple4}set} or
  a {\color{Purple4}def} to a {\color{Purple4}guess}.
...
\begin{frame}
  ...
  {\color{Green4}prior knowledge} by adding restraints in quadrature
...
\begin{alltt}
    \scriptsize
     {\color{Purple4}guess} S0sqr = 0.8
     {\color{SteelBlue4}path}(1, S02 = {\color{Cornsilk4}max}(0.6, {\color{Cornsilk4}min}(1.0, S0sqr) ) )
\end{alltt}
...
\begin{alltt}
    \scriptsize
     {\color{Purple4}guess}    S0sqr = 0.8
     {\color{Purple4}set}      scale = 2000
     {\color{Purple4}restrain} S0sqr\_res = scale * {\color{Cornsilk4}penalty}(S0sqr, 0.6, 1.0)
...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    \begin{alltt}
    \scriptsize
     {\color{Purple4}guess}    S0sqr = 0.9
     {\color{Purple4}set}      S0sqr\_known = 0.876
     {\color{Purple4}set}      scale = 2000
     {\color{Purple4}restrain} S0sqr\_res = scale * (S0sqr - SOsqr\_known)
    \end{alltt}
  \end{block}
...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    \begin{alltt}
    \scriptsize
     {\color{Purple4}set}      valence = 2
     {\color{Purple4}set}      rij     = 1.734  {\color{Blue4}# R'ij for Fe(2+)-O}
     {\color{Purple4}set}      rnot    = 2.14
     {\color{Purple4}set}      scale   = 1000
     {\color{Purple4}guess}    delr    = 0.0
     {\color{Purple4}restrain} bvs     = valence - 6 * exp( (rij - (rnot+delr)) / 0.37 )
    \end{alltt}
  \end{block}
...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    \begin{alltt}
    \scriptsize
     {\color{Purple4}guess}      r\_short     = 2.32
     {\color{Purple4}guess}      r\_long      = 2.45
     {\color{Purple4}restrain}   r\_short\_res = scale * penalty(r\_short, 2.30, 2.34)
     {\color{Purple4}restrain}   r\_long\_res  = scale * penalty(r\_short, 2.43, 2.47)
    \end{alltt}
  \end{block}
...
%\definecolor{MYcolor}{RGB}{51,51,178}   %% Warsaw default
...
\begin{frame}
  ...
  \begin{center}
    \includegraphics[width=0.7\linewidth]{artemis/mkw.png}
...
\begin{frame}
  ...
  \begin{center}
    ...
    {\color{Magenta3}Restraints} are managed on the Guess, Def, Set
    page, like any other parameter and will be properly used in the
    fit.  A restraint depends upon a {\color{Green4}def} or
    {\color{Purple4}guess} parameters -- something that changes during
    the fit.
  \end{center}
\end{frame}

\begin{frame}
  \frametitle{Now you know all the tricks!}

  \begin{block}{\LARGE Your assignment:}
    \begin{center}
      \LARGE Use 'em all!\\[1ex]  Do great data analysis!
    \end{center}
  \end{block}
\end{frame}
