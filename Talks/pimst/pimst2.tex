...
\begin{frame}
  ...
  \begin{textblock*}{0.6\linewidth}(0pt,19\TPVertModule)%
    \tiny%
    The derivation on the following N pages is similar to an example by Matt
    Newville
...
\begin{frame}
  ...
  \begin{align}
    ...
    \big[
    1 + A(E)\big|\langle \alert{\Delta f}| \mathcal{H}|i\rangle\big|
    + C.C.
    \big]\notag\\[2ex]
    \mu(E) =&\, \mu_0(E) \cdot (1+\chi(E))\notag\\[2ex]
    \therefore\> \chi(E) \propto&\,\Big(\big|\langle \alert{\Delta f}|
...
\begin{frame}
  ...
  \begin{equation}
    k = \sqrt{\frac{2m_e(E-E_0)}{\hbar^2}} \simeq \frac{\sqrt{E-E_0}}{3.81}\notag
  \end{equation}
